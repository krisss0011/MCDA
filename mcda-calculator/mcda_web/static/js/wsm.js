function fetchWsmResults() {
    fetch('/api/waspas-results/')
        .then(response => response.json())
        .then(data => {
            displayResults(data[0]); 
        })
        .catch(error => console.error('Error fetching the WSM results:', error));
}
...
function updateWeight(index, newWeight) {
    ...
    const url = `http://127.0.0.1:8000/api/calculation/?method=waspas&weights=${weights.join(',')}`;
