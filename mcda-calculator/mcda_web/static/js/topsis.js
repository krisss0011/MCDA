function fetchTopsisResults() {
    fetch('/api/topsis-results/')
        .then(response => response.json())
        .then(data => {
            displayResults(data[0]); 
        })
        .catch(error => console.error('Error fetching the Topsis results:', error));
}
...
function updateWeight(index, newWeight) {
    ...
    const url = `http://127.0.0.1:8000/api/calculation/?method=topsis&weights=${weights.join(',')}`;
